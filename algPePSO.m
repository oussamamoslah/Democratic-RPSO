function [listout] = algPePSO(simrobot,list,matrix,step)
...
%                                         improve=1;  % swarm of this robot did improve
        ...
        yc = message.yc;
            ...
            if (getcrashed(simrobot) == 1)
                ...
                x1=round(x0+vx);
                y1=round(y0+vy);
                simrobot = setcrashed(simrobot, '0');
                simrobot = setvel(simrobot,[0 -2]);
%                 head = gethead(simrobot);
%                 newhead = head - 90 ;
%                 if (newhead<0)
%                     newhead  = 360 + newhead ;
%                 end
%                 simrobot = sethead(simrobot,newhead);
